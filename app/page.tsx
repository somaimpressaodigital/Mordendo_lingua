...
function TeamPanel({ team, side, isLast, showGames, onUpdateScore, onUpdateGames, onNameChange }: any) {
  return (
    <div className={`flex flex-col items-center justify-center p-4 md:p-12 bg-slate-900/20 ${!isLast ? 'border-r' : ''} border-sky-900/50 relative transition-all duration-500 hover:bg-slate-900/40 group overflow-hidden`}>
      ...
      <div className="grid grid-cols-4 md:flex md:flex-nowrap gap-2 md:gap-3 mt-4 md:mt-8 w-full md:max-w-none md:justify-center">
        <button 
          onClick={() => onUpdateScore(1)}
          className="aspect-square md:w-16 md:h-16 rounded-lg md:rounded-full bg-slate-100 flex items-center justify-center text-sm md:text-2xl font-bold hover:bg-white text-slate-950 transition-all active:scale-90 shadow-lg shadow-white/5"
        >
          +1
        </button>
        <button 
